...
while ($true) {
    ...
    switch ($choice) {
        ...
        "2" {
            ...
            Start-Process "msedge.exe" -ArgumentList "--app=http://localhost:3000/ --window-size=1100,720"
            cmd /c "npm run dev"
            Read-Host "`n  Press Enter to return to menu..."
        }
        "3" {
        ...
        "6" {
            ...
            Write-Host "  ======================================================================" -ForegroundColor Cyan
            $ok = Invoke-BuildX64 -version $currentVer
            if ($ok) {
                Invoke-BuildArm64 -version $currentVer
            } else {
                Write-Host "[-] x64 build failed! Aborting ARM64 build." -ForegroundColor Red
            }
            Read-Host "  Press Enter to return to menu..."
        }
        "7" {
            ...
            switch ($bumpType) {
                "1" { cmd /c "node scripts/bump-version.js patch"; Read-Host "`n  Press Enter to return to menu..." }
                "2" { cmd /c "node scripts/bump-version.js minor"; Read-Host "`n  Press Enter to return to menu..." }
                "3" { cmd /c "node scripts/bump-version.js major"; Read-Host "`n  Press Enter to return to menu..." }
                "4" {
        ...
        "8" {
            ...
            if ($releaseExe) {
                ...
                $targetExe = "src-tauri\target\debug\bukaake.exe"
            }
